...
def calc_technical_indicators(klines):
    ...
    if result['kdj']['cross'] == '金叉':
        score += 25
        signals.append("KDJ金叉→短线买入信号")
    elif result['kdj']['cross'] == '死叉':
        score -= 25
        signals.append("KDJ死叉→短线卖出信号")
    ...
    if result['macd']['cross'] == '金叉':
        score += 20
        signals.append("MACD金叉→趋势转多")
    elif result['macd']['cross'] == '死叉':
        score -= 20
        signals.append("MACD死叉→趋势转空")
    ...
    if vp == '放量上涨':
        score += 10
        signals.append("放量上涨→多方强势")
    elif vp == '放量下跌':
        score -= 10
        signals.append("放量下跌→空方强势")
    elif vp == '缩量上涨':
        score -= 5
        signals.append("缩量上涨→上涨乏力")
...
def fetch_single_stock(s, sina_data=None):
    ...
    if klines and len(klines) >= 5:
        ...
        # 计算短线技术指标
        indicators = calc_technical_indicators(klines)
        if indicators:
