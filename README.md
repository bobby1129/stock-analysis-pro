...
入口自带交易日检查（`core/trading_calendar.py`）：周末/节假日休市时打印 `NON_TRADING_DAY: <原因>` 并退出（exit 0），避免用旧数据生成复盘。cron任务见此输出应回复 `[SILENT]` 完全静默。
...
自动生成适合抖音发布的竖屏数据可视化内容（1080×1920），包含11张图 + 11个动画视频：

1. **个股成交额TOP10**（1张）— 全市场按成交额排序（新浪数据源），含排名变化箭头（↑↓）；另有"盘后情报局"IP角标（Vol.期号）+ 今日一句研判
   - **1b. 龙虎标尺动画视频**（1个mp4，约18秒）— 同源数据的动画版：中央排名标尺，10支股票大卡片左右交替飞入、放大展示6项数据后落座缩小定格（名称+箭头/成交额+环比），NEW金色角标
2. **概念板块资金意图矩阵**（5张）— akshare同花顺全量373概念（无采样偏差），价格×资金四象限：
   ...
   - **2b. 板块资金全景图动画视频**（1个mp4，约14秒）— 同源数据动画版：标题居中放大后缩回顶部，四张象限卡片依次中央放大展示再落座四宫格，最后说明行+研判一起淡入
   - **2c. 资金意图矩阵四榜动画视频**（4个mp4，每条约7秒）— p1失血/p2对倒/p3主攻/p4潜伏各自一个视频，同一动画逻辑：标题（日期+榜名）居中放大后落座顶部，副标题→研判→榜单轻微先后淡入；视频版不展示「盘后情报局」角标与门槛页脚（规则存档DAILY_CONTENT.md）
3. **新进成交额TOP50**（1张或多张）— 首次进入成交额前50的股票，支持智能分页：
   - ≤15只：1页展示完
   - >15只：每页10只，分多页展示（标题显示"第X/Y页"）
   - 含IP角标+自动研判句、量比/换手列（腾讯源）、信号标注（⚠巨量绿柱=量比>2且下跌，警惕次日低开；🟢资金进场=量比>1.2且上涨）
   - **3b. 交易额新进TOP50动画视频**（1个mp4，约21秒）— 同源数据动画版：标题居中放大（折两行）后缩回顶部，条目依次飞入中间放大展示（名称/涨幅/成交额），落位后显示完整7列信息
4. **异常信号捕捉**（4张）— 放量滞涨/缩量新高/放量急拉 + 连板梯队（腾讯量比 + akshare涨停池）
   - **4b. 异常信号捕捉动画视频**（4个mp4，每条约7秒）— 「日期+异常信号捕捉」固定顶部全程展示；栏目名（图标+名称，不含括号说明）开场中央放大后落位到卡片标题，随后榜单行→页脚轻微先后淡入；数据只读 anomaly 缓存（与图片同源，缺失回退最近缓存）

一键生成所有图片：
```bash
python scripts/daily_content/run_all.py
```
入口自带交易日检查：周末/节假日休市自动跳过（对比上证指数行情日期与今天），避免用旧数据生成内容。
